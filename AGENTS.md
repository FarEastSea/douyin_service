...
## Web Configuration Baseline

- Web 设置中心是所有配置的唯一权威来源。进程环境变量、`.env` 和代码默认值只能作为首次启动或持久化载体，不能覆盖网页已保存值。
- 新增任何配置时，必须同时提供网页配置入口、持久化读写和运行时动态读取；禁止只增加环境变量或要求手工编辑服务端文件。
- 业务逻辑必须在实际使用配置时获取最新网页值，禁止把读取结果保存为长期模块常量、单例快照或硬编码派生值。
- 配置更新后，各功能应在下一次调用或调度周期平滑切换；若外部连接确实需要重建，应由系统自动重建，不能要求用户把手工重启作为常规生效方式。
- 下载目录变更必须同步更新已有 `DownloadTask.file_path`、`DownloadTask.temp_file_path`、`DownloadHistory.file_path` 和 `XDownloadTask.download_dir`，并保持相对目录结构与预览链接可用。
- 读取配置的优先级固定为：网页持久化值 > 首次启动值 > 代码默认值。缓存只能用于加速，必须可失效且不得成为权威来源。
