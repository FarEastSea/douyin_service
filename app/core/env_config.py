...
ENV_FIELDS: List[EnvField] = [
    ...
    EnvField(key="NOTIFY_ENABLED", label="启用通知中心", group="通知", default="false"),
    EnvField(key="NOTIFY_ON_NEW_WORKS", label="发现新作品时通知", group="通知", default="true"),
    EnvField(key="NOTIFY_ON_DOWNLOAD_FAILURE", label="下载失败时通知", group="通知", default="true"),
    EnvField(key="NOTIFY_ON_RISK", label="触发抖音风控时通知", group="通知", default="true"),
    EnvField(key="NOTIFY_ON_SUBSCRIPTION_FAILURE", label="订阅检查异常时通知", group="通知", default="true"),
    EnvField(key="NOTIFY_DEDUPE_SECONDS", label="相同通知去重时间（秒）", group="通知", default="300"),
    EnvField(key="WEBHOOK_ENABLED", label="启用 Webhook", group="通知", default="false"),
    EnvField(key="WEBHOOK_URL", label="Webhook 地址", group="通知", default=""),
    EnvField(key="WEBHOOK_SECRET", label="Webhook 签名密钥", group="通知", default="", secret=True, help="可选；使用 HMAC-SHA256 生成 X-Signature-SHA256"),
    EnvField(key="BARK_ENABLED", label="启用 Bark", group="通知", default="false"),
    EnvField(key="BARK_SERVER_URL", label="Bark 服务地址", group="通知", default="https://api.day.app"),
    EnvField(key="BARK_DEVICE_KEY", label="Bark Device Key", group="通知", default="", secret=True),
    EnvField(key="EMAIL_ENABLED", label="启用邮件", group="通知", default="false"),
    EnvField(key="SMTP_HOST", label="SMTP 主机", group="通知", default=""),
    EnvField(key="SMTP_PORT", label="SMTP 端口", group="通知", default="465"),
    EnvField(key="SMTP_USERNAME", label="SMTP 用户名", group="通知", default=""),
    EnvField(key="SMTP_PASSWORD", label="SMTP 密码", group="通知", default="", secret=True),
    EnvField(key="SMTP_FROM", label="发件地址", group="通知", default=""),
    EnvField(key="SMTP_TO", label="收件地址", group="通知", default="", help="多个地址用英文逗号分隔"),
    EnvField(key="SMTP_SECURITY", label="SMTP 安全模式", group="通知", default="ssl", help="可选 ssl、starttls 或 none"),
    EnvField(key="GOTIFY_ENABLED", label="启用 Gotify", group="通知", default="false"),
    EnvField(key="GOTIFY_SERVER_URL", label="Gotify 服务地址", group="通知", default=""),
    EnvField(key="GOTIFY_TOKEN", label="Gotify 应用 Token", group="通知", default="", secret=True),
...
def _validate_env_value(key: str, value: str) -> None:
    ...
    expected_engine = _DOWNLOAD_ENGINE_VALUES.get(key)
    if expected_engine and value.strip().lower() != expected_engine:
        raise ValueError(f"{field.label}当前仅支持 {expected_engine}")
