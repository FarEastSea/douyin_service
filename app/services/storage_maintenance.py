...
async def build_storage_repair_plan(
    ...
    *,
    ...
    for target in targets:
        ...
        try:
            if issue_type not in {"missing_record", "zero_byte_file", "partial_file", "orphan_file"}:
            ...
            if issue_type in {"missing_record", "zero_byte_file"}:
                if record is None:
                    raise ValueError("关联记录不存在")
                recorded_path = _record_path(record)
                if not recorded_path:
                    raise ValueError("关联记录已不再指向文件")
                recorded_resolved, _ = _resolved_under(root, recorded_path)
                if recorded_resolved != path:
                    raise ValueError("关联记录路径已经变化")
            if issue_type == "missing_record":
                if path.exists():
