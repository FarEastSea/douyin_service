...
const store = useAppStore(), reports = ref<any[]>([]), loading = ref(false)
const latest = computed(() => reports.value[0])
async function load() {
  loading.value = true
  try { reports.value = (await api<any>('/authors/reports/subscriptions?limit=20')).items || [] }
...
<template>
  <section class="workspace-card update-workspace">
    ...
    <div class="metric-grid"><article><Users /><div><strong>{{ latest?.total_authors || store.stats.subscribed_authors }}</strong><span>订阅作者</span></div></article><article><Clock3 /><div><strong>{{ latest?.checked_authors || 0 }}</strong><span>本轮已检查</span></div></article><article><Sparkles /><div><strong>{{ latest?.new_works || 0 }}</strong><span>发现新作品</span></div></article><article><RefreshCw /><div><strong>{{ latest?.remaining_authors || 0 }}</strong><span>等待续检</span></div></article></div>
    <div class="timeline" :class="{ loading }">
      <article v-for="report in reports" :key="report.id" class="timeline-item"><i :data-tone="report.status" /><div class="timeline-head"><strong>{{ report.summary || '订阅检查' }}</strong><span class="status subtle" :data-tone="report.status">{{ report.status }}</span></div><p>{{ report.checked_authors }} 位已检查 · {{ report.success_authors }} 位成功 · {{ report.warning_authors + report.failed_authors }} 位异常</p><footer><time>{{ report.started_at ? new Date(report.started_at).toLocaleString() : '时间未知' }}</time><span>{{ report.trigger_type === 'manual' ? '手动触发' : '自动调度' }}</span></footer></article>
