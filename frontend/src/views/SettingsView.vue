...
async function init() {
  const requests = [loadRuntime(), loadArchiveRules(), loadDouyinAccount(), loadPlatformCredentialStatus(), loadAll(), loadProcess(), loadReadiness(), loadPlatformReadiness(), refreshStorageAudit(true), loadLogs(), loadUpdateInfo()]
...
async function copyLogs() { await navigator.clipboard.writeText(filteredLogs.value.map(item => `${new Date(item.ts * 1000).toLocaleString()} [${item.level}] [${item.source}]${item.event_code ? ` [${item.event_code}]` : ''}${item.correlation_id ? ` [请求 ${item.correlation_id}]` : ''} ${item.msg}\n${item.detail || ''}${Object.keys(item.context || {}).length ? `\n上下文: ${JSON.stringify(item.context)}` : ''}`).join('\n\n')); store.notify('日志已复制（敏感值已脱敏）') }
  ...
  <section class="workspace-card settings-workspace">
    ...
    <div v-else-if="tab === 'operations'" class="settings-panel operations-panel">
      ...
      <div class="storage-report">
        <div class="storage-report-header"><div><strong>存储巡检与可恢复维护</strong><span v-if="storageAuditBusy">{{ storageAuditState.phase === 'files' ? '正在扫描文件' : '正在核对数据库记录' }} · 已处理 {{ storageAuditState.progress?.scanned_records || 0 }} 条记录、{{ storageAuditState.progress?.scanned_files || 0 }} 个文件；离开页面后仍会继续</span><span v-else-if="storageAudit">已扫描 {{ storageAudit.scanned_records }} 条记录、{{ storageAudit.scanned_files }} 个文件 · {{ displayDateTime(storageAudit.checked_at) }}</span><span v-else-if="storageAuditState.status === 'failed'">上次扫描失败：{{ storageAuditState.error }}</span><span v-else>点击“扫描存储”后显示结果；任务在后台执行，刷新页面不会丢失</span></div><div class="header-actions" v-if="storageAudit"><button class="btn ghost compact" :disabled="storageRepairBusy || !storageRepairTargets.length" @click="previewStorageRepair">{{ storageRepairBusy ? '核验中…' : '预演处理' }}</button><button v-if="storageRepairPlan" class="btn primary compact" :disabled="storageRepairBusy || !eligibleRepairTargets.length" @click="applyStorageRepair">确认处理 {{ eligibleRepairTargets.length }} 项</button></div></div>
        <template v-if="storageAudit">
          <div class="storage-metrics"><article><b>{{ storageAudit.disk.used_percent }}%</b><span>磁盘已用</span></article><article><b>{{ storageAudit.relinkable_records?.length || 0 }}</b><span>可修复旧路径</span></article><article><b>{{ storageAudit.missing_records.length }}</b><span>记录缺文件</span></article><article><b>{{ storageAudit.partial_files.length }}</b><span>陈旧临时文件</span></article><article><b>{{ storageAudit.zero_byte_files.length }}</b><span>空文件</span></article><article><b>{{ storageAudit.orphan_files.length }}</b><span>未关联媒体</span></article></div>
...
.validation-evidence strong { color:var(--muted); font-size:10px; }
