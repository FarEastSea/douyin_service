...
import { Activity, Archive, BellRing, Clipboard, Cookie, Database, Play, QrCode, RefreshCw, Save, Server, Settings2, Square, Trash2 } from '@lucide/vue'
...
const xhsBrowser = ref<any>({ installed: false, state: 'stopped', logged_in: false, message: '尚未检查' })
const xhsQr = ref<any>(null), xhsBrowserBusy = ref(false)
...
const readinessLabel = (name: string) => ({ configuration: '应用配置', database: '数据库', redis: 'Redis', worker: 'Celery Worker', beat: 'Celery Beat', xhs_collector: '小红书采集' }[name] || name)
...
async function loadXhsBrowserStatus() { try { xhsBrowser.value = await api('/platform-downloads/xhs/browser/status') } catch (error: any) { xhsBrowser.value = { installed: false, state: 'error', logged_in: false, message: error.message || '浏览器状态不可用' } } }
async function startXhsBrowser() { xhsBrowserBusy.value = true; try { xhsBrowser.value = await api('/platform-downloads/xhs/browser/start', { method: 'POST' }); store.notify(xhsBrowser.value.message || '小红书浏览器已启动') } catch (error: any) { store.notify(error.message || '启动小红书浏览器失败', 'error') } finally { xhsBrowserBusy.value = false } }
async function showXhsQr() { xhsBrowserBusy.value = true; try { xhsQr.value = await api('/platform-downloads/xhs/browser/qrcode', { method: 'POST' }); if (xhsQr.value.is_logged_in) { xhsBrowser.value.logged_in = true; store.notify('小红书浏览器已经登录') } } catch (error: any) { store.notify(error.message || '生成登录二维码失败', 'error') } finally { xhsBrowserBusy.value = false } }
...
async function init() {
  ...
  if (route.path.startsWith('/xhs')) requests.push(loadXhsBrowserStatus())
...
<template>
  <section class="workspace-card settings-workspace">
    ...
    <div v-if="tab === 'account' && route.path.startsWith('/xhs')" class="settings-panel xhs-browser-panel"><header><QrCode /><div><h3>小红书受管浏览器</h3><p>作者主页采集使用独立 Chromium 登录态；Jenkins 部署不会覆盖会话</p></div><button class="btn ghost" :disabled="xhsBrowserBusy" @click="loadXhsBrowserStatus"><RefreshCw :size="15" />刷新状态</button></header><article class="xhs-browser-card"><div class="account-health"><strong>浏览器会话</strong><span :data-status="xhsBrowser.logged_in ? 'healthy' : 'degraded'">{{ xhsBrowser.logged_in ? '已登录' : xhsBrowser.state === 'running' ? '待扫码' : '未启动' }}</span></div><p>{{ xhsBrowser.message }}</p><div class="header-actions"><button class="btn ghost" :disabled="xhsBrowserBusy" @click="startXhsBrowser"><Play :size="15" />启动浏览器</button><button class="btn primary" :disabled="xhsBrowserBusy" @click="showXhsQr"><QrCode :size="15" />扫码登录</button></div><div v-if="xhsQr?.image_data_url" class="xhs-qr"><img :src="xhsQr.image_data_url" alt="小红书登录二维码" /><small>{{ xhsQr.expires_at ? `二维码有效至 ${new Date(xhsQr.expires_at).toLocaleTimeString()}` : xhsQr.message }}</small></div></article></div>
...
.xhs-browser-panel { margin-top: 14px; }
.xhs-browser-card { display: grid; gap: 12px; padding: 14px; border: 1px solid var(--line); border-radius: 10px; background: var(--surface-2); }
.xhs-qr { display: grid; justify-items: start; gap: 8px; }
.xhs-qr img { width: 220px; max-width: 100%; padding: 8px; border-radius: 10px; background: #fff; }
.xhs-qr small { color: var(--muted); }
