...
async function create() {
  if (!input.value.trim()) return store.notify(`请输入 ${platformName.value} 用户主页或用户名`, 'error')
  try {
...
async function remove(task: PlatformTask) {
  if (confirm(`确定删除 @${task.source_key} 的任务记录？本地媒体文件会保留。`)) await action(task, '', 'DELETE')
}
...
<template>
  <section class="workspace-card">
    <header class="workspace-header"><div><p class="eyebrow">MULTI-PLATFORM PIPELINE</p><h2>{{ platformName }} 下载任务</h2><span>通过平台注册适配器管理用户主页媒体</span></div><button class="btn ghost" @click="load"><RefreshCw :size="16" />刷新</button></header>
    <form class="command-bar" @submit.prevent="create"><span class="media-icon">{{ definition?.icon_text || 'M' }}</span><input v-model="input" :placeholder="`输入 ${platformName} 用户主页 URL 或 @用户名…`" /><button class="btn primary">开始下载</button></form>
